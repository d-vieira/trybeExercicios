...
const advertisingByAge = (ages) => {
  const check = ages.some((age) => age < 18);
  check ? console.log(`ChocoTrybe, melhor que console.log`) : console.log(`Beba Trybeer, suave como um log de erro vazio`);
}

advertisingByAge(arrayIdades);
advertisingByAge(arrayIdades2);
...
function findIncompleteProjects () {
  return estudantes.some((estudante) => estudante.status < 80) ? 'Bora pra mentoria de projetos!' : 'Pode abrir o suco de laranja que temos 100% de aprovação!';
}
console.log(findIncompleteProjects());
